...
def AttentionResNetCifar10(shape=(32, 32, 3), n_channels=32, n_classes=10):
    ...
    x, output_soft_mask = attention_block(x, encoder_depth=2)

    x2 = residual_block(x, input_channels=128, output_channels=256, stride=2)  # 8x8
    x2, _ = attention_block(x2, encoder_depth=1)

    x2 = residual_block(x2, input_channels=256, output_channels=512, stride=2)  # 4x4
    x2, _ = attention_block(x2, encoder_depth=1)

    x2 = residual_block(x2, input_channels=512, output_channels=1024)
    x2 = residual_block(x2, input_channels=1024, output_channels=1024)
    x2 = residual_block(x2, input_channels=1024, output_channels=1024)

    x2 = AveragePooling2D(pool_size=(4, 4), strides=(1, 1))(x2)  # 1x1
    x2 = Flatten()(x)
    output = Dense(n_classes, activation='softmax')(x2)

    model = Model(input_, output)
    mask_model = Model(input_, output_soft_mask)
